...
def update_todo(self):
    ...
    try:
        todo_id = int(str_id)
        new_title = InputUtil.input("Judul Baru")
        status = InputUtil.input("Status (1=Selesai, 0=Belum)")

        if status not in ["0", "1"]:
            print("[!] Status tidak valid.")
            return

        is_finished = True if status == "1" else False
        self.todo_service.update_todo(todo_id, new_title, is_finished)
...
def sort_todo(self):
    ...
    print("2. Berdasarkan Judul")
    print("3. Berdasarkan Status")

    choice = InputUtil.input("Pilih")
    if choice == "1":
        self.todo_service.sort_todo("id")
    elif choice == "2":
